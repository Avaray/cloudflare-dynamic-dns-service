...
export const ValidateConfig = async (cfg: Config, issues: string[]) => {
  ...
  cfg.domain.length > 0 &&
    !/[\w\d-]+\.[\w\d]+/.test(cfg.domain) &&
    issues.push(`Domain name is not valid (${cfg.domain})`);
  ...
  cfg.keyType.length > 0 &&
    !/^(token|key)$/.test(cfg.keyType) &&
    issues.push(`Invalid API key type. Choose between 'token' and 'key'.`);
...
// Function to generate config file from interface Config
export const generateConfig = (cfg: Config) => {
  // todo
};
